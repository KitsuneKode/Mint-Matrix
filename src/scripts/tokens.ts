...
export async function getTokenAccounts(walletToQuery: PublicKey, network: string = "mainnet-beta") {
  // Establish connection to the cluster
  let rpcEndpoint;
  if(network === "mainnet-beta") {
    rpcEndpoint = "https://solana-mainnet.g.alchemy.com/v2/JxeseX6EEjWMsFemERtHiIEUumVqW7iG";
    
  }
    else {
       rpcEndpoint = "https://solana-devnet.g.alchemy.com/v2/7ZqHHHCbJtNgLU6XWFl4mUjLbwkyPenB"
    }
  ...
  for (const [i, account] of Array.from(accounts.entries())) {
    ...
    if (i < token22Length) {
      ...
      metadata = await getTokenMetadata(connection, mint, "confirmed", TOKEN_PROGRAM_ID);
    }
